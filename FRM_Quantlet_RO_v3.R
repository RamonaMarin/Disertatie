...
date_start_fixed = 20150105
date_end_fixed =   20240131
...
ro_names= read.csv(file = 
                     "C:\\Users\\ramon\\Desktop\\FACULTATE\\MASTER\\DISERTATIE\\FRM RO\\FRM\\Input\\RO\\tickers.csv", header = TRUE)
...
stock_prices<-stock_prices %>%  dplyr::select(dput(as.character("ticker"))|
                                        one_of(dput(as.character(select$ticker))))

valid_prices<-stock_prices %>% 
  
  summarise_all(~ sum(!is.na(.)))%>% dplyr::select(-'ticker')
...
mktcap<-mktcap %>% dplyr::select(dput(as.character("ticker"))|
                            one_of(dput(as.character(select$ticker))))
...
all_prices[, (M_stock+2):(M+1)] =
  all_prices[, (M_stock+2):(M+1)] %>% na.locf(fromLast=T)
...
if(any(all_prices[, -1] <0, na.rm = TRUE)) {
  warning("Există prețuri ale acțiunilor <= 0")
}
...
for (t in N0:N1) { 
  ...
  for (k in 1:M_t) { 
    ...
    est_lambimida = abs(data.matrix(est$lambda[which(est$Cgacv == min(est$Cgacv))]))
  ...
  write.csv(adj_matrix, paste0(output_path, "/Adj_Matrix/adj_matrix_", 
...
for (t in 1:(N_h-1)) {
  adj0 = read.csv(file=paste0(output_path, "/Adj_Matrix/adj_matrix_", 
                              gsub("-", "", names(FRM_history)[t]), ".csv"), 
                  header = TRUE, sep = "," , row.names = 1)
...
save.image(file='BVB_16_04_2024_5%.RData')
